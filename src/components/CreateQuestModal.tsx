...
import { QuestDifficulty, RewardItem, DIFFICULTY_EXP, REWARD_CATEGORIES } from '../types'
import { getTodayEnd } from '../utils/time'
...
export function CreateQuestModal() {
  const { createQuest, setShowCreateModal } = useGuildStore()
  ...
  const [deadline, setDeadline] = useState(() => {
    const end = new Date(getTodayEnd())
    return `${end.getFullYear()}-${String(end.getMonth() + 1).padStart(2, '0')}-${String(end.getDate()).padStart(2, '0')}T23:59`
  })
  const [expReward, setExpReward] = useState(DIFFICULTY_EXP.C)
  ...
  return (
    <div className="modal-overlay fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowCreateModal(false)}>
      <div
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
            <label className="block text-sm font-bold text-wood-700 mb-1">
              委托标题 <span className="text-guild-crimson">*</span>
            </label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="例如：讨伐拖延大魔王"
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              placeholder="详细描述委托内容..."
              rows={3}
              className="w-full px-3 py-2 bg-parchment-100 border border-parchment-300 rounded text-wood-800 placeholder-wood-400 focus:outline-none focus:border-guild-gold resize-none"
            />
          </div>
              ...
              {(['D', 'C', 'B', 'A', 'S'] as QuestDifficulty[]).map((d) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded text-sm font-bold transition-all ${
                    difficulty === d
                      ? 'bg-guild-gold text-wood-900 shadow-md'
                      : 'bg-parchment-200 text-wood-600 hover:bg-parchment-300'
                  }`}
                >
                  {d}
                </button>
          ...
          <div className="mb-4 flex items-center gap-2">
            <input
              ...
              className="accent-guild-crimson"
            />
            <label htmlFor="penalty" className="text-sm text-wood-700">
              开启惩罚模式（失败扣除 20% 经验）
            </label>
          </div>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={addReward}
                className="text-sm text-guild-gold hover:text-guild-darkGold"
              >
                + 添加奖励
              </button>
            </div>
            ...
            {rewards.map((reward, index) => (
              <div key={reward.id} className="flex items-center gap-2 mb-2">
                    ...
                    <div className="absolute top-12 left-0 z-10 bg-parchment-100 border border-parchment-300 rounded p-2 shadow-lg grid grid-cols-8 gap-1 w-64">
                      {COMMON_EMOJIS.map((emoji) => (
                        <button
                          key={emoji}
                          onClick={() => {
                            updateReward(index, { emoji })
                            setShowEmojiPicker(null)
                          }}
                          className="text-xl hover:bg-parchment-200 rounded p-1"
                        >
                          {emoji}
                        </button>
                      ))}
                ...
                <input
                  ...
                  onChange={(e) => updateReward(index, { quantity: Math.max(1, Math.min(999, Number(e.target.value))) })}
                  min={1}
                  max={999}
                ...
                <button
                  ...
                  className="text-wood-400 hover:text-guild-crimson"
                >
                  ×
                </button>
              </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={() => setShowCreateModal(false)}
              className="flex-1 py-2 bg-parchment-200 text-wood-600 rounded font-bold hover:bg-parchment-300 transition-colors"
            >
              取消
            </button>
            <button
